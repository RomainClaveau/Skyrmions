...
########################
import numpy as np
from math import asinh, atan, sqrt, pi, fmod
...
def F(m, h, gamma, alpha):
    return gamma / (1 + alpha**2) * np.cross(h, m) + gamma * alpha / (1 + alpha**2) * np.cross(m, np.cross(h, m)) - D * gamma * alpha / (1 + alpha**2) * m
        ...
        with open('dynamics.dat', 'w') as f:
            while t < max:
                ...
                if fmod(c, 100) == 0: # Printing 1 point for 100 points computed
                    ...
                    print "%.15f %f %f %f" % (t, m_vec[0], m_vec[1], m_vec[2])
                    f.write("%.15f %f %f %f\n" % (t, m_vec[0], m_vec[1], m_vec[2]))
